"""Rank documents by distinct shared words plus distinct shared adjacent word pairs."""
...
MODEL_NAME = "1.9.1-unigram-bigram-overlap"
...
def tokenize(text):
    """Return distinct word and adjacent-pair features, each worth one point."""
    words = TOKEN_PATTERN.findall(text.lower())
    return {(word,) for word in words} | set(zip(words, words[1:]))
...
def retrieve(query, postings, top_k=1000):
    ...
    # Each shared feature contributes one point; ties use descending document IDs.
