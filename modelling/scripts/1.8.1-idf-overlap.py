"""Rank documents by the sum of smoothed IDF weights of distinct shared words."""
...
def build_index(corpus_path):
    ...
    weights = {
        term: score(len(documents), document_count)
        for term, documents in postings.items()
    }
    return postings, weights, document_count


def score(document_frequency, document_count):
    return math.log((document_count + 1) / (document_frequency + 1)) + 1


def retrieve(query, postings, weights, top_k=1000):
    if top_k < 1:
    ...
    for term in sorted(terms):
        for doc_id in postings.get(term, ()):
            contributions[doc_id].append(weights[term])
...
def main(argv=None):
    ...
    try:
        ...
        postings, weights, document_count = build_index(corpus_path)
        ...
        with tempfile.TemporaryDirectory(prefix=".retrieval-", dir=output_dir) as temp:
            ...
            with temporary_path.open("w", encoding="utf-8") as stream:
                for query_id, text in read_records(queries_path, "query_id"):
                    results = retrieve(text, postings, weights, args.top_k)
