"""Rank documents by shared words divided by distinct document words."""
...
MODEL_NAME = "1.2.1-document-coverage"
...
def build_index(corpus_path):
    ...
    return postings, document_lengths


def score(overlap, query_length, document_length):
    return overlap / document_length if document_length else 0.0


def retrieve(query, postings, document_lengths, top_k=1000):
    if top_k < 1:
    ...
    scores = (
        (doc_id, score(count, len(terms), document_lengths[doc_id]))
        for doc_id, count in overlaps.items()
    )
...
def main(argv=None):
    ...
    try:
        ...
        postings, document_lengths = build_index(corpus_path)
        ...
        with tempfile.TemporaryDirectory(prefix=".retrieval-", dir=output_dir) as temp:
            ...
            with temporary_path.open("w", encoding="utf-8") as stream:
                for query_id, text in read_records(queries_path, "query_id"):
                    results = retrieve(text, postings, document_lengths, args.top_k)
